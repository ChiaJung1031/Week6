...
@app.route('/error')
def errorpage():
   if session["signup"] == "帳號已經被註冊":
     errmsg = request.args.get("message","帳號已經被註冊")
     if errmsg == "帳號已經被註冊":
        return  render_template('error.html',errmsg="帳號已經被註冊")
     else:
        return  render_template('error.html',errmsg=errmsg)
   elif session["signup"] == "申請帳號資料請填寫完整": 
     errmsg = request.args.get("message","申請帳號資料請填寫完整")
     if errmsg == "申請帳號資料請填寫完整":
         return  render_template('error.html',errmsg="申請帳號資料請填寫完整")
     else:
         return  render_template('error.html',errmsg=errmsg)
   else:
     ...
     # print(adr)
      ...
      if len(account) == 0 or len(psword)== 0 :
         ...
         if len(myresult) == 0:
            ...
            return redirect(url_for('memberpage', message=result))

@app.route('/member/<message>')
def memberpage(message):
